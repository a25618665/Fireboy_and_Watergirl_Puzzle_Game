...
{   if(character.Top() + character.Height() >= chest_and_key.Top()
	&& character.Left() + character.Width() >= chest_and_key.Left()&&phase == 3
	  ) {
    ...
    if ( character.Left() + 10 >= door[0].Left()&& character.Top() <= door[0].Top() + door[0].Height() &&phase == 5

		){
		door[0].SelectShowBitmap(1);
	     }
	if ( character.Left() + 10 >= door[1].Left() && character.Top() <= door[2].Top() + door[2].Height() &&phase == 5
		)
    {
		door[1].SelectShowBitmap(1);
	}
	if ( character.Left() +  10 >= door[2].Left() && character.Top() <= door[2].Top()+door[2].Height() && phase == 5
		) 
	{
...
void CGameStateRun::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	int y_now = character.Top();
	int x_now = character.Left();
	if (nChar == VK_UP) {
		y_now -=10;
		character.SetTopLeft(x_now, y_now);
